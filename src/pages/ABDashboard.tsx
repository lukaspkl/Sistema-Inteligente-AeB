...
import PerdasDialog from "@/components/PerdasDialog";
import PerdasValidadeDialog from "@/components/PerdasValidadeDialog";

export default function ABDashboard() {
  const { toast } = useToast();
  const [aiPrompt, setAiPrompt] = useState("");
  const [isProcessingAI, setIsProcessingAI] = useState(false);
  const [perdasDialogOpen, setPerdasDialogOpen] = useState(false);
  const [perdasValidadeDialogOpen, setPerdasValidadeDialogOpen] = useState(false);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button 
            onClick={() => setPerdasValidadeDialogOpen(true)}
            variant="outline"
          ...
          <Button 
            onClick={() => setPerdasDialogOpen(true)}
            variant="outline"
      ...
      <PerdasDialog 
        open={perdasDialogOpen} 
        onOpenChange={setPerdasDialogOpen}
        onSuccess={fetchMetricas}
      />

      <PerdasValidadeDialog 
        open={perdasValidadeDialogOpen} 
        onOpenChange={setPerdasValidadeDialogOpen}
        onSuccess={fetchMetricas}
      />
    </div>
  );
